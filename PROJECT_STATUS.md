...
- **2026-09-09** — Reproduced and repaired surrender/end partial saves: failed completion retains the active timer and draft, notification/activity transitions follow durable success, and starting another session cannot replace one whose end failed. Two injected-write disk/reopen/retry tests plus all 208 package tests pass. No installed build, signing, schema or provider change; build 25 remains unreleased under #52's historical CloudKit, hosted and signed-device gates.
- **2026-09-09** — Prepared build 25 source with device-only distraction note and keyword storage, pre-mirror legacy migration, failure-safe capture/pause, and explicit missing-device-note wording. All 206 package tests pass, including 11 synthetic disk/migration/failure tests and an exact old-source store fixture. Hosted native review [34321290511](https://github.com/Significant-Hobbies/anchor/actions/runs/34321290511) was blocked before any job steps by GitHub account payment/spending limits; it is not a native test result. This is not installed or distributed; historical CloudKit copy removal, pending legacy exports, signed mixed-version/device qualification and notarization remain open in [issue 52](https://github.com/Significant-Hobbies/anchor/issues/52). See [the bounded qualification receipt](docs/qualification/private-notes-2026-09-09/README.md).
